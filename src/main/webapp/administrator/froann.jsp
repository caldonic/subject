...
<%
    ...
    List<FroannVO> list = froannSvc.getAll();
              ...
              <div class="col-sm-4">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item">
                    <a href='froann/froannadd.jsp'>新增</a>
                  </li>
...
<!--                   <form id="quickForm"> -->
     	...
     	<%@ include file="./froann/page1.file" %>                
		<c:forEach var="aaa" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
<%-- 		${aaa} --%>
		<tr>
			<td>${aaa.froannno}</td>
<%-- 			<td>account<input type="hidden" value="${aaa.admno}"/></td> --%>
			<td>${aaa.admno}</td>
			<td>${aaa.froanndesc}</td>
			<td>${aaa.froannsdate}</td>
			<td>${aaa.froannedate}</td>
<%-- 			<td><fmt:formatDate   value="${empVO.froannsdate}" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
<%-- 			<td><fmt:formatDate   value="${empVO.froannedate}" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
			<td>${aaa.froannstatus == 0 ? "on" : "off"}</td>			
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/administrator/froann.do" style="margin-bottom: 0px;">
			     <input type="submit" value="修改">
			     <input type="hidden" name="froannno"  value="${aaa.froannno}">
    ...
    <%@ include file="./froann/page2.file" %>
