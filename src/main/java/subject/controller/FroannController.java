...
//		System.out.println("abc");
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			req.setAttribute("errorMsgs", errorMsgs);
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/administrator/froann/select_page.jsp");
					failureView.forward(req, res);
					return;//程式中斷
				}
				...
				String url = "/administrator/froann/listOneEmp.jsp";
...
//				Integer	froannstatus = null;//不知道上架狀態怎麼寫
//				try {
//froannstatus = Integer.valueOf(req.getParameter("froannstatus").trim());
...
//				}
				...
				froannVO.setFroannstatus(froannstatus);//狀態不知道怎麼寫
				...
				/***************************2.開始修改資料*****************************************/
				FroannService froannSvc = new FroannService();
				froannVO = froannSvc.updateFroann(froannno, admno, froanndesc, froannsdate, froannedate, froannstatus);//froannstatus
...
//			}
			...
			froannVO.setFroannstatus(froannstatus);//狀態不知道怎麼寫
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("froannVO", froannVO); // 含有輸入格式錯誤的froannVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/administrator/froann/froannadd.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/***************************2.開始新增資料***************************************/
				FroannService froannSvc = new FroannService();
				froannVO = froannSvc.addFroann( admno, froanndesc, froannsdate, froannedate, froannstatus);//froannstatus
						
				/***************************3.新增完成,準備轉交(Send the Success view)***********/
//				String url = "/froann/listAllEmp.jsp";
